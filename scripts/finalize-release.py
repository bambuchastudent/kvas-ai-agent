...
<html lang="ru"><head>
  ...
  <style>
    ...
    .answer{{padding:17px 19px;border-radius:16px;background:#0c3454;font-weight:850}}.primary{{display:flex;align-items:center;justify-content:space-between;gap:18px;margin-top:25px;padding:17px 21px;border-radius:999px;background:var(--accent);color:#04111d;font-weight:900;text-decoration:none}}
    ...
    .links{{display:flex;flex-wrap:wrap;gap:10px;margin-top:22px}}.links a{{padding:11px 15px;border:1px solid #47769b;border-radius:999px;color:#9edcff;text-decoration:none}}
  </style>
</head><body><main>
  ...
  <section class="card"><h2>Что бот умеет</h2><ul class="features">
    <li><strong>Короткий рецепт</strong><br>Сухари, сусло, сахар, закваска, брожение, ПЭТ и охлаждение.</li>
    <li><strong>Разбор партии</strong><br>Температура, время, поверхность, запах, вкус и газированность.</li>
    <li><strong>Пожелания</strong><br>Идеи и ошибки превращаются в задачи следующего релиза.</li>
    <li><strong>Фотографии</strong><br>Фото кваса и подписи попадают в тот же общий PR.</li>
  </ul></section>
...
for required in (BOT_URL, "@kvassistent_bot", f"ВЕРСИЯ {ONES}", "Ай да какой ты квас задумал", "Фото кваса"):
    if required not in telegram_html:
        raise RuntimeError(f"Telegram page finalization missing: {required}")

print(f"finalized KVASSISTENT version {ONES}: smart Telegram recipe, batch help and release feedback")
